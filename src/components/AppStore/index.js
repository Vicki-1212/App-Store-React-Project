...
class AppStore extends Component {
  state = {searchItem: tabsList[0].tabId, searchName: ''}

  updateState = tabId => {
    this.setState({searchItem: tabId})
  }
  ...
  searchAppList = () => {
    ...
    const searchResults = appsList.filter(eachAppList =>
      eachAppList.appName.toLowerCase().includes(searchName.toLowerCase()),
    )
    return searchResults
  }

  getAppList = searchResults => {
    const {searchItem} = this.state
    const filteredAppList = searchResults.filter(
      eachAppList => eachAppList.category === searchItem,
    )
    return filteredAppList
  }
  render() {
    const {searchItem, searchName} = this.state
    const searchResults = this.searchAppList()
    const filteredAppList = this.getAppList(searchResults)
        ...
        <ul className="tab-container">
          {tabsList.map(eachList => (
            <TabItem
              ...
              isActive={eachList.tabId === searchItem}
            />
          ))}
        </ul>
        <ul className="appList-container">
          {filteredAppList.map(eachAppList => (
            <AppItem appList={eachAppList} key={eachAppList.appId} />
          ))}
        </ul>
      </div>
    )
